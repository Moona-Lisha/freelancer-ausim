...
wp.customize.controlConstructor['interface-tabs'] = wp.customize.Control.extend({
            ...
            if( typeof $_GET['autofocus[control]'] !== 'undefined' && $_GET['autofocus[control]'] !== ''){
                jQuery('li[id^="customize-control-sidebars_widgets"]').on(
                    'DOMNodeInserted', function () {
                        setTimeout( function(){
                            jQuery('.custom-customizer-tab > label.' + $_GET['autofocus[control]']).trigger('click');
                        } , 100 );
                    });
            }
        } );
    ...
    init: function () {
        ...
        setTimeout( function() {
            jQuery('li[id^="customize-control-sidebars_widgets"]').each(function () {
                jQuery(this).on(
                    'DOMNodeInserted', function () {
                        jQuery('.custom-customizer-tab.active > label').trigger('click');
                    });
            });
        }, 100 );
    ...
    handleClick: function () {
        ...
        jQuery(container).find('.custom-customizer-tab').on( 'click', function () {
            jQuery( this ).parent().find('.custom-customizer-tab').removeClass('active');
